...
static void build_slists(char *proxy)
{
   if (slist) {
   ...
   if (slist_nocache) {
      curl_slist_free_all(slist_nocache);
      slist_nocache = NULL;
   }

   char *cernvm_id;
#ifdef FUSE_CLIENT
   char *version_prefix = "Fuse ";
#else
   char *version_prefix = "libcvmfs ";
#endif
   char *header_proxy;
   ...
   {
      int len_custom_header = strlen("User-Agent: cvmfs ") + strlen(version_prefix) + strlen(VERSION) + 1 +
                              strlen(getenv("CERNVM_UUID")) + 1 + strlen(header_proxy) + 1;
      cernvm_id = alloca(len_custom_header);
      snprintf(cernvm_id, len_custom_header, "User-Agent: cvmfs %s%s %s %s", version_prefix, VERSION, getenv("CERNVM_UUID"), header_proxy);
   } else {
      int len_custom_header = strlen("User-Agent: cvmfs ") + strlen(version_prefix) + strlen(VERSION) + 1 +
                              strlen("anonymous") + 1 + strlen(header_proxy) + 1;
      cernvm_id = alloca(len_custom_header);
      snprintf(cernvm_id, len_custom_header, "User-Agent: cvmfs %s%s anonymous %s", version_prefix, VERSION, header_proxy);
   }
...
int curl_download_stream_nocache(const char *url, FILE *f, unsigned char digest[SHA1_DIGEST_LENGTH], int probe_hosts, int compressed)
{
   ...
   curl_easy_setopt(curl_default, CURLOPT_HTTPHEADER, slist_nocache);
   curl_easy_setopt(curl_default, CURLOPT_PRIVATE, (void *)1);
   if (probe_hosts) {
      result = curl_download_stream_probing(url, f, digest, compressed);
   } else {
      result = curl_download_stream_unprotected(url, f, digest, compressed);
   }
   curl_easy_setopt(curl_default, CURLOPT_PRIVATE, (void *)0);
   curl_easy_setopt(curl_default, CURLOPT_HTTPHEADER, slist);
...
int curl_download_mem_nocache(const char *url, struct mem_url *p_mem_url, int probe_hosts, int compressed)
{
   ...
   curl_easy_setopt(curl_mem, CURLOPT_HTTPHEADER, slist_nocache);
   curl_easy_setopt(curl_mem, CURLOPT_PRIVATE, (void *)1);
   if (probe_hosts) {
      result = curl_download_mem_probing(url, p_mem_url, compressed);
   } else {
      result = curl_download_mem_unprotected(url, p_mem_url, compressed);
   }
   curl_easy_setopt(curl_mem, CURLOPT_PRIVATE, (void *)0);
   curl_easy_setopt(curl_mem, CURLOPT_HTTPHEADER, slist);
...
static void __attribute__((constructor)) initialize (void)
{
   ...
   curl_easy_setopt(curl_default, CURLOPT_PRIVATE, (void *)0); // Use proxies
   ...
   curl_easy_setopt(curl_tee, CURLOPT_PRIVATE, (void *)0);
