...
which  must be converted to a human readable form with @ref dttools_sha1_string.
...
int dttools_sha1_file(const char *filename, unsigned char digest[SHA1_DIGEST_LENGTH]);

/** Convert an SHA1 digest into a printable string.
@param digest A binary digest returned from @ref dttools_sha1_file.
@returns A static pointer to a human readable form of the digest.
*/

const char *dttools_sha1_string(unsigned char digest[SHA1_DIGEST_LENGTH]);
